...
| [columns.py](columns.py) | `day_to_columns` — flattens a `.day` file into two columns (date, flow), one row per calendar day. Missing days are kept as `-99.99` rather than dropped, so the series has no unexplained gaps. `python -m disag.columns`, or `--columns` on a `python -m disag` run to write it alongside the `.day`. Every web `/disag` run publishes it too. |
