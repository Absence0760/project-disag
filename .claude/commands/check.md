...
Run a parallel audit on the working diff, aggregate findings, and report. Advisory only — you don't apply fixes here, the user decides which to land.
...
- It does NOT replace `/safe-edit`'s coder ↔ reviewer loop. `/safe-edit` is for risky edits (Lambda handler, infra, file-format parsers, the disag algorithm) that warrant the 2-3x cost of a fix-and-re-review cycle. `/check` is the lighter pre-commit gate.
...
Send a single message with three Agent tool calls (whichever of these agents are present in `.claude/agents/` — the bare minimum is `test-gap-checker` and `doc-hygiene-checker`; add `code-reviewer` if it's been ported):

- `code-reviewer` (if present) — prompt: "Review the working diff against this project's conventions (root `CLAUDE.md`, `disag/CLAUDE.md`, `exceed/CLAUDE.md`, `web/README.md`). Output the strict format from your spec."
- `test-gap-checker` — prompt: "Audit the working diff for missing unit + e2e test surface. Output the format from your spec."
- `doc-hygiene-checker` — prompt: "Audit the working diff against this repo's doc set. Output which docs need updating."

Parallel because they're independent — all of them only `git diff` + `Read` files.

### 4. Aggregate

When all agents return, build a single short report:
...
### Recommendation
<one of:>
- All agents came back clean — ready to commit.
...
If `code-reviewer` isn't present, omit that section and note it once: "(code-reviewer not yet ported)".
