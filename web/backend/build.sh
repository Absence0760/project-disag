...
# Defensive secret + cruft exclusions. The list is non-exhaustive —
# anything that looks credential-shaped or admin-secret-shaped that
# might land in disag/ or exceed/ later should be stripped pre-emptively
# so a future accidental commit doesn't ride along.
# Docs (CLAUDE.md, README.md) are stripped too — operator notes are
# noise in production, not source code.
find "$BUILD_DIR" -type f \( \
    -name '.env' -o -name '.env.*' \
    -o -name '*.pem' -o -name '*.key' -o -name '*.p8' \
    ...
    -o -name '.sops.yaml' -o -name 'secrets.yaml' -o -name 'secrets.json' \
